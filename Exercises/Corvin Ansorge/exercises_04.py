...
def execute_program(commands, input_value):
    op_pointer = 0
    output = None
    while op_pointer < len(commands):
        instruction = str(commands[op_pointer]).zfill(5)
        opcode = int(instruction[-2:])
        param_modes = [int(instruction[2]), int(instruction[1]), int(instruction[0])]
        ...
        if opcode == 1:
            commands[commands[op_pointer + 3]] = get_value(op_pointer + 1, param_modes[0]) + get_value(op_pointer + 2,
                                                                                                       param_modes[1])
            op_pointer += 4
        elif opcode == 2:
            commands[commands[op_pointer + 3]] = get_value(op_pointer + 1, param_modes[0]) * get_value(op_pointer + 2,
            ...
            if get_value(op_pointer + 1, param_modes[0]) != 0:
                op_pointer = get_value(op_pointer + 2, param_modes[1])
            else:
                op_pointer += 3
        elif opcode == 6:
            if get_value(op_pointer + 1, param_modes[0]) == 0:
            ...
            commands[commands[op_pointer + 3]] = 1 if get_value(op_pointer + 1, param_modes[0]) < get_value(
                op_pointer + 2, param_modes[1]) else 0
            op_pointer += 4
        elif opcode == 8:
            commands[commands[op_pointer + 3]] = 1 if get_value(op_pointer + 1, param_modes[0]) == get_value(
                op_pointer + 2, param_modes[1]) else 0
            ...
            raise ValueError(f"Unknown opcode {opcode} at position {op_pointer}")

    return output


result = execute_program(commands.copy(), 5)
print(result)
