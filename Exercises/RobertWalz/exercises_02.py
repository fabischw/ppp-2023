...
def get_refs(list_of_nums, current_pos):
    """gets references to relevant entrys

    Args:
        list_of_nums (list): list in format: [opcode, first_ref, second_ref, save_ref]
...
def addWrite(list_of_nums, current_pos):
    ...
    Args:
        ...
        currentPos (number): starting pos in list_of_nums
    """
    first_ref, second_ref, pos_to_write_to = get_refs(list_of_nums, current_pos)

    result = list_of_nums[first_ref] + list_of_nums[second_ref]
    list_of_nums[pos_to_write_to] = result


def mutiplyWrite(list_of_nums, current_pos):
...
def func(*args, cast_func=float):
    ...
    for element in args:
        ...
        try:
            numbers.append(cast_func(element))
        except ValueError:
