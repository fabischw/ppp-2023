...
# - 5: jump-if-true: if the first parameter is non-zero, it sets the instruction pointer to
# the value from the second parameter. Otherwise, it does nothing.
# - 6: jump-if-false: if the first parameter is zero, it sets the instruction pointer to 
# the value from the second parameter. Otherwise, it does nothing.
# - 7: less than: if the first parameter is less than the second parameter, it stores 1
# in the position given by the third parameter. Otherwise, it stores 0.
# - 8: equals: if the first parameter is equal to the second parameter, it stores 1
# in the position given by the third parameter. Otherwise, it stores 0.
#
# caution: since these opcodes expect a variable number of parameters the instruction
# pointer after executing an instruction should no longer always
# increase by four. Instead, it should increase  according to the number of parameters that
# instruction expects.
# caution: numbers in your storage, as well as inputs can be negative!
# If, however, the opcode directly manipulates the instruction pointer (5 and 6) there 
# should be no additional modification of the instruction pointer.
# The next instruction to execute is stored directly at the location indicated by the
# parameter to these opcodes.
#
# Additionally, you need to support two different parameter modes: position mode (mode 0),
# and immediate mode (mode 1).
# Position mode: Opcode arguments are memory addresses. If an argument has the value 18 
#you fetch the 'calculation value'
#   from the memory at address 18. This is the mode you already know from last time.
# Immediate mode: In immediate mode a parameter is directly interpreted as a value. 
# If the first argument to the 'sum' opcode is 8
...
#   - any further digits contain the parameter mode of the parameters, reading digits from
# right-to-left
#     and parameters in-order from left to right. Any unspecified digits default to 0 
# (position mode).
#     NOTE: parameters for the target address of a write operation
#  (e.g. the third parameter of the 'sum' or 'multiply' opcode)
...
#   Then, from right to left the next digit is '0', indicating that the first parameter 
# is in position mode.
#   The next digit is '1' indicating the second parameter is in immediate mode.
#   The next digit is not present, defaulting to '0' so the third parameter is again in
#  position mode.
#   No further parameter modes need to be determined as the multiply instruction accepts
#  3 parameters.
...
test1 =  [3,9,8,9,10,9,4,9,99,-1,8] # test whether the input is equal to 8 (using position mode)
test2 =  [3,3,1107,-1,8,3,4,3,99] # test whether the input is less than 8 (using immediate mode)
test3 =  [3,3,1105,-1,9,1101,0,0,12,4,12,99,1] # test whether the input is 0 using jump instructions
#  
#
# Finally, run you code for the following instructions; when asked for input provide 
# the number '5'. The program should print a single number when executed.
...
def execute(memory: [int], debug=0, debug_input=0):
    counter = 0
    while(True):
        command = [*str(memory[counter])]
        command_len = len(command)
        if(command_len > 1):
            instruction = "".join(command[command_len-2:])
            mode = command[:command_len-2]
        else:
            instruction = "0" + command[0]
            mode = []
        match instruction:
            case "01" :
                reg1 = memory[counter+1] if (len(mode) >= 1 and mode[-1] == "1") else memory[memory[counter+1]]
                reg2 = memory[counter+2] if (len(mode) >= 2 and mode[-2] == "1") else memory[memory[counter+2]]
                reg3 = memory[counter+3]    
                memory[reg3] = reg1 + reg2
                counter += 4
            case "02" :
                reg1 = memory[counter+1] if (len(mode) >= 1 and mode[-1] == "1") else memory[memory[counter+1]]
                reg2 = memory[counter+2] if (len(mode) >= 2 and mode[-2] == "1") else memory[memory[counter+2]]
            ...
            case "03" :
                if(debug == 0):
                    user_input = ''
                    user_input = input("")
                    memory[memory[counter+1]] = int(user_input) # error handling
                else:
                    memory[memory[counter+1]] = debug_input
                counter += 2
            case "04" :
                print(memory[memory[counter+1]])
                counter += 2
            case "05":
                reg1 = memory[counter+1] if (len(mode) >= 1 and mode[-1] == "1") else memory[memory[counter+1]]
                reg2 = memory[counter+2] if (len(mode) >= 2 and mode[-2] == "1") else memory[memory[counter+2]]
...
execute(test3, 1, 4) # output = 1
print("-> output = 1")
execute(test1, 1, 5) # output = 0 | equals to 8
print("-> output = 0\n-------------------")
execute(test1, 1, 8) # output = 1
print("-> output = 1\n-------------------")
execute(test2, 1, 0) # output = 1 | less than 8
print("-> output = 1\n-------------------")
execute(test2, 1, 9) # output = 0
print("-> output = 0\n-------------------")
execute(test3, 1, 0) # output = 0 | is not zero
print("-> output = 0\n-------------------")
execute(commands, 1, 5)
print("-> output = 16694270")
