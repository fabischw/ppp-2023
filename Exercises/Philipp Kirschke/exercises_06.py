...
def update_sizes(commands, sizes, stack):
    for command in commands:
        ...
        if command.startswith("$ cd"):
            ...
            if destination == "..":
                ...
                current_path = f"{stack[-1]}_{destination}" if stack else destination
...
file_path = Path(str(working_dir) + "/data/terminal_record.txt")
...
update_sizes(commands, sizes, stack)

needed_size = 30000000 - (70000000 - sizes["/"])
threshold_size = next(size for size in sorted(sizes.values()) if size > needed_size)

print(sum(n for n in sizes.values() if n <= 100000))  # task 1
print(threshold_size)  # task 2
