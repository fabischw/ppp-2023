...
def computer(commands):
    ...
    current_index = 0
    while running:
        if commands[current_index] == 99:
            running = False
        else:
            try:
                a = commands[commands[current_index+1]]
                b = commands[commands[current_index+2]]
...
def task2(*args):
    ...
    for elements in args:
        ...
        if elements != "nan":#handle nan string as "Not a number" - is not a number
            try:
                test_number = float(elements)# Test if current string is convertable to a float which should include ALL numbers
