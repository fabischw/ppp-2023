...
def add(index,mode1,mode2,mode3):
    commands[commands[index + 3]] = commands[mode1] + commands[mode2]
    return index + 4

def multiply(index,mode1,mode2,mode3):
    commands[commands[index + 3]] = commands[mode1] * commands[mode2]
    return index + 4

def inp(index,mode1,mode2,mode3):
    try: commands[mode1]=int(input("input an integer:"))
    except: raise RuntimeError("wrong input")
    return index + 2

def outp(index,mode1,mode2,mode3):
    print(">>>",commands[mode1])
    return index + 2

def jump_if_true(index,mode1,mode2,mode3):
    if commands[mode1] != 0: return commands[mode2]
    return index + 3

def jump_if_false(index,mode1,mode2,mode3):
    if commands[mode1] == 0: return commands[mode2]
    return index + 3

def less_than(index,mode1,mode2,mode3):
    if commands[mode1] < commands[mode2]: commands[mode3] = 1
    else: commands[mode3] = 0
    return index + 4
        
def equals(index,mode1,mode2,mode3):
    if commands[mode1] == commands[mode2]: commands[mode3] = 1
    else: commands[mode3] = 0
    return index + 4
...
def checkmode(index,mode):
    if mode == "0": return commands[index]
    elif mode == "1": return index
    else:raise ValueError("wrong mode parameter")

def simulated_computer(commands, index=0):
    while True: 
        opcode = str(commands[index]).zfill(5)
        numopcode = opcode[3:]
        if numopcode == "99": break
        elif opcodedict.get(numopcode) != None: index = opcodedict[numopcode](index,checkmode(index+1,opcode[2]),checkmode(index+2,opcode[1]),checkmode(index+3,opcode[0]))
