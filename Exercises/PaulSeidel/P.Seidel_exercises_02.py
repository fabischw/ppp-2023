...
#commands = [1, 1, 1, 4, 99, 5, 6, 0, 99] #shorter List for troubleshooting

def computerSim(listCS):
    for counter in range(0,int(len(listCS)),4):   #going through every block of 4 elements
        #print(f'checking the {counter} element')#troubleshooting
        match listCS[counter]:
            ...
            case 99:    #halt
                #print(f'detected operator 99')#troubleshooting
                return listCS[0]
            case _: #in case of an invalid Operator
                #print(f'detected invalid operator')#troubleshooting
                return f'Error: The {counter} element of the given list is an invalid operator!'

print(computerSim(commands)) #execute the function and print the return statement
...
def collector():    #collecting all input-Strings and putting them in a list
    ...
    while temp != 'break':
        temp = input()
        if temp != 'break':
            list.append(temp)   #to avoid the last element being 'break'
    print(f'collected list: {list}\n')
    return list

def executor():
    list = collector()
    #print('list is finished')#troubleshooting
    length = len(list)
    listDigit = []
    listChar = []
    #print(f'length is {length}')#troubleshooting
    for counter in range(length):
        #print(f'for loop counter: {counter}')#troubleshooting
        temp = list[counter]
        if temp.isdigit():  #checks if the string is made out of numbers from 0 to 9
            listDigit.append(temp)
        if len(temp) == 1:
            listChar.append(temp)
    print(f'list of numbers: {listDigit}\n')
    print(f'list of characters: {listChar}')
