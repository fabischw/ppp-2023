...
""" 0 is position mode number at index at pointer, 1 is immediate mode number directly at index
first 3 digits müssen umgedreht werden oder es müssen die anderen Befehle zugewiesen werden"""
#
# Here's an example:
...
# commands =  [3,3,1105,-1,9,1101,0,0,12,4,12,99,1] #-- test whether the input is 0 using jump instructions
...
def sim_computer(program):
    def get_param_value(param_mode, param):
        return program[param] if param_mode == 0 else param

    def execute_instruction(pointer):
        instruction = str(program[pointer]).zfill(5)
        opcode = int(instruction[-2:])
        param_modes = [int(x) for x in instruction[:-2]][::-1]

        if opcode == 1:  
            param1 = get_param_value(param_modes[0], program[pointer + 1])
            param2 = get_param_value(param_modes[1], program[pointer + 2])
            ...
            output_value = get_param_value(param_modes[0], program[pointer + 1])
            return output_value, pointer + 2

        elif opcode == 5: 
            param1 = get_param_value(param_modes[0], program[pointer + 1])
            param2 = get_param_value(param_modes[1], program[pointer + 2])
            return pointer + 3 if param1 == 0 else param2
            ...
            param1 = get_param_value(param_modes[0], program[pointer + 1])
            param2 = get_param_value(param_modes[1], program[pointer + 2])
            program[program[pointer + 3]] = 1 if param1 < param2 else 0
            return pointer + 4
    ...
    while pointer is not None:
        result = execute_instruction(pointer)
