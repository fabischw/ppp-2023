...
# commands = [3,3,1105,-1,9,1101,0,0,12,4,12,99,1]
def calculate(arr):
    element = 0
    while element < len(arr):
        ziffern = list(str(arr[element]))
        opcode_list = ziffern[-2::1]
        opcode = ""
        parameter_mode = ziffern[-3::-1]
        for zahl in range(0,len(opcode_list)):
            opcode += opcode_list[zahl]
        if int(opcode) == 1:
            fehlende_parameter(3,parameter_mode)
            input1 = arr[mode_check(parameter_mode,arr,element,0)]
            input2 = arr[mode_check(parameter_mode,arr,element,1)]
            position = mode_check(parameter_mode,arr,element,2)
            arr = opcode1(input1,input2,position,arr)
            element+=4
        elif int(opcode) == 2:
            fehlende_parameter(3,parameter_mode)
            input1 = arr[mode_check(parameter_mode,arr,element,0)]
            input2 = arr[mode_check(parameter_mode,arr,element,1)]
            position = mode_check(parameter_mode,arr,element,2)
            arr = opcode2(input1,input2,position,arr)
            element+=4
        elif int(opcode) == 3:
            fehlende_parameter(1, parameter_mode)
            position = mode_check(parameter_mode,arr,element,0)
            arr = opcode3(position, arr)
            element+=2
        elif int(opcode) == 4:
            fehlende_parameter(1,parameter_mode)
            position = mode_check(parameter_mode,arr,element,0)
            opcode4(position,arr)
            element+=2
        elif int(opcode) == 5:
            fehlende_parameter(2,parameter_mode)
            input1 = arr[mode_check(parameter_mode,arr,element,0)]
            position = mode_check(parameter_mode,arr,element,1)
            element = opcode5(input1,position,arr,element)
        elif int(opcode) == 6:
            fehlende_parameter(2,parameter_mode)
            input1 = arr[mode_check(parameter_mode,arr,element,0)]
            position = mode_check(parameter_mode,arr,element,1)
            element = opcode6(input1,position,arr,element)
        elif int(opcode) == 7:
            fehlende_parameter(3,parameter_mode)
            input1 = arr[mode_check(parameter_mode,arr,element,0)]
            input2 = arr[mode_check(parameter_mode,arr,element,1)]
            position = mode_check(parameter_mode,arr,element,2)
            arr = opcode7(input1,input2,position,arr)
            element+=4
        elif int(opcode) == 8:
            fehlende_parameter(3,parameter_mode)
            input1 = arr[mode_check(parameter_mode,arr,element,0)]
            input2 = arr[mode_check(parameter_mode,arr,element,1)]
            position = mode_check(parameter_mode,arr,element,2)
            arr = opcode8(input1,input2,position,arr)
            element+=4
        elif int(opcode) == 99:
            break
        else:
            print("Fehler an Stelle: ", element, "Nummer: ", arr[element])
            break

def fehlende_parameter(anzahl_benoetigt, aktuell):
    """
    verlaengert die Liste an parameter modes auf die anzahl an benoetigten
    args: benoetigte Anzahl für den opcode, aktuelle Liste der parameter modes
    return: überarbeitete Liste
    """
    if len(aktuell) < anzahl_benoetigt:
        verlaengerung = 3-len(aktuell)
        for fehlende_modes in range(0,verlaengerung):
            aktuell.append("0")
        return aktuell
    
def mode_check(parameter_mode, arr, element, parameter_stelle):
    """
    ueberprueft, ob der parameter mode 1 oder 0 ist und gibt benoetigten stelle zurueck
    args: aktueller mode, liste, element
    return: benoetigte stelle in liste
    """
    mode = parameter_mode[parameter_stelle]
    if mode == '1':
        stelle = element+parameter_stelle+1
    else:
        stelle = arr[element+parameter_stelle+1]
    return stelle        

def opcode1(input1,input2,position,arr):
    """
    addition of input1 and input2
    args: input1, input2, position, list
    return: edited list
    """
    ergebnis = input1 + input2
    arr[position] = ergebnis
    return arr

def opcode2(input1,input2,position,arr):
    """
    multiplication of input1 and input2
    args: input1, input2, position, list
    return: edited list
    """
    ergebnis = input1 * input2
...
def opcode3(position,arr):
    """
    takes an input from the console and writes it to a given position
    args: position, list
    return: edited list
    """
    user_input = input('Please enter a number')
    arr[position] = int(user_input)
    return arr

def opcode4(position,arr):
    """
    prints an integer from a given position from a list
    args: position, list
    """
    print(arr[position])

def opcode5(input1,position,arr,element):
    """
    checks if input1 is equal to 0.
    If not it sets the position of the indicator to the given point
    args: input1, position, list
    return: edited position
    """
    if input1 != 0:
        new_position = arr[position]
    else:
        new_position = element +3
    return new_position

def opcode6(input1,position,arr,element):
    """
    checks if input1 is equal to 0.
    If it is, sets the position of the indicator to the given point
    args: input1, position, list
    return: edited position
    """
    if input1 == 0:
...
def opcode7(input1,input2,position,arr):
    """
    if input1 is less than input2, updates given position in array to 1
    else updates it to 0
    args: input1, input2, position, list
    return: edited list
    """
    if input1 < input2:
...
def opcode8(input1,input2,position,arr):
    """
    if input1 is equal to input2, updates given position in array to 1
    else updates it to 0
    args: input1, input2, position, list
    return: edited list
    """
