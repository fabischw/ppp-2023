...
def less_than(first_pos, second_pos):
    if first_pos<second_pos:
        return 1
    else:
        return 0
    
def equals(first_pos, second_pos):
    if first_pos == second_pos:
        return 1
    else:
        return 0
...
def wierd_computer(commands):
    i=0
    while i <= len(commands):
        instruction = list(reversed(str(commands[i])))
        while len(instruction) < 5:
            instruction.append("0")
            #print(instruction)
        command = int(instruction[1] + instruction[0])
        #print (command)
        if command == 99:
            break
        ...
        match command: 
            case 1:
                save_pos= thierd
                commands[save_pos] = addition(commands[first],commands[second])
                i+=4
                #print(commands[first], commands[second])
                #print(addition(commands[first],commands[second]))
                #print(commands[save_pos], save_pos)
            case 2:
                save_pos=thierd
                commands[save_pos]=multiplication(commands[first],commands[second])
                i+=4
            case 3:
                save_pos=first
                commands[save_pos] = do_input()
                i+=2
                #print(save_pos,commands[save_pos])
            case 4:
                save_pos=first
            ...
            case 7:
                save_pos=thierd
                commands[save_pos]=less_than(commands[first],commands[second])
                i+=4
            case 8:
                save_pos=thierd
                commands[save_pos]=equals(commands[first],commands[second])
                i+=4
            case other :
                print("error accured")
                break
...
print(wierd_computer(commands_1))