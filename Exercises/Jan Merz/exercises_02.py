...
def computer_sim(arr):
    position = 0
    while(True):
        if(arr[position] == 1):
            read_location1 = arr[position+1]
            read_location2 = arr[position+2]
            write_location = arr[position+3]
            arr[write_location] = arr[read_location1] + arr[read_location2]
            position += 4
        elif(arr[position] == 2):
            read_location1 = arr[position+1]
            read_location2 = arr[position+2]
...
splitArgs("0", "-4254", "0.00000000001", "ä", "pß", "?")
