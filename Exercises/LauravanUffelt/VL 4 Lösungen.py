...
def comp_sim(intake):
    ...
    def opcodes(count):
        ...
        match opcode:
            ...
            case 3:  # read a single integer as input and save it to the position given by its only parameter.
                user_input = int(input("Please enter a number:"))
                adress = intake[count+1]
                intake[adress] = user_input
                return count + 2

            case 4:  # output the value of the single parameter for this opcode.
                adress = intake[count+1]
                print(f"{intake[adress]}")
                return count + 2
