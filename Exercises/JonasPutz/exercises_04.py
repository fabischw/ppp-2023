...
def _command_input(current_index, memory_stream, params):
    def is_integer(string):
        try: i = int(string)
        except ValueError: return False, 0
        return True, i
        
    id_result = memory_stream[current_index + 1]

    integer_Output = is_integer(input("Enter input as integer: "))
    while not integer_Output[0]:
        integer_Output = is_integer(input("Wrong input. Try again: "))
    
    memory_stream[id_result] = integer_Output[1]
...
def _command_halt(current_index, memory_stream, params):
    return -1


_opt_code_dictionary = {
    1: _command_add,
    2: _command_multiply,
    3: _command_input,
    4: _command_output,
...
def mini_computer(memory_stream):
    """ mini computer simulation program.
        ...
        This function will run the _commands inputed as the argument.
        
        Parameters
        ----------
        memory_stream (int[])
            the _commands used by the computer. This will be modified by the function on runtime  
    ...
    while current_index >= 0 and current_index < command_length:
        params = memory_stream[current_index]
        opt_code = params % 100
        params //= 100
        if opt_code in _opt_code_dictionary:
            current_index = _opt_code_dictionary[opt_code](current_index, memory_stream, params)
        else:
