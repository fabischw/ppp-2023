...
def get_args(op_pointer, storage, num_args):
    ...
    for arg_pos in range(num_args):
        args.append(storage[op_pointer + arg_pos + 1] if modes[arg_pos] else storage[storage[op_pointer + arg_pos + 1]])
    return args


def op_addition(op_pointer, storage):
    in1, in2 = get_args(op_pointer, storage, 2)
    save_address = storage[op_pointer + 3]
    storage[save_address] = in1 + in2
    op_pointer += 4
    return op_pointer, 'ok'


def op_multiplication(op_pointer, storage):
    in1, in2 = get_args(op_pointer, storage, 2)
    save_address = storage[op_pointer + 3]
    storage[save_address] = in1 * in2
    op_pointer += 4
...
def op_input(op_pointer, storage):
    in_val = input('Please enter a number: ')
    try:
        in_val = int(in_val)
    except ValueError:
        raise ValueError('The entered value is not a number!')
    save_address = storage[op_pointer + 1]
    storage[save_address] = in_val
    op_pointer += 2
    return op_pointer, 'ok'


def op_output(op_pointer, storage):
    read_location = storage[op_pointer + 1]
    out_val = storage[read_location]
    print(out_val)
    op_pointer += 2
    return op_pointer, 'ok'


def op_jump_if_true(op_pointer, storage):
    truth_val, jump_address = get_args(op_pointer, storage, 2)
    op_pointer = jump_address if truth_val else op_pointer + 3
    return op_pointer, 'ok'


def op_jump_if_false(op_pointer, storage):
    truth_val, jump_address = get_args(op_pointer, storage, 2)
    op_pointer = jump_address if not truth_val else op_pointer + 3
    return op_pointer, 'ok'


def op_less_than(op_pointer, storage):
    in1, in2 = get_args(op_pointer, storage, 2)
    save_address = storage[op_pointer + 3]
    storage[save_address] = 1 if in1 < in2 else 0
    op_pointer += 4
    return op_pointer, 'ok'


def op_equals(op_pointer, storage):
    in1, in2 = get_args(op_pointer, storage, 2)
    save_address = storage[op_pointer + 3]
    storage[save_address] = 1 if in1 == in2 else 0
    op_pointer += 4
...
def op_halt(op_pointer, storage):
    return op_pointer, 'halt'


op_code_dict = {1: op_addition,
                2: op_multiplication,
                3: op_input,
                4: op_output,
                5: op_jump_if_true,
...
def compute(storage):
    op_pointer = 0
    while op_pointer < len(storage):
        op_code = storage[op_pointer] % 100
        op_pointer, status = op_code_dict[op_code](op_pointer, storage)
        match status:
            case 'ok':
                pass
            case 'halt':
                return storage[0]
            case _:
                raise Exception('Unknown error! Exiting program.')
